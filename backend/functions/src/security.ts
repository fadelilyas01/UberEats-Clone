...
interface DeviceInfo {
    ...
    device_id: string; // Android ID, IDFV (iOS), or browser fingerprint
    model: string;
    os_version: string;
    app_version: string;
}
...
export const verifyDeviceOnLogin = functions.https.onCall(async (data, context) => {
    if (!context.auth) {
        throw new functions.https.HttpsError('unauthenticated', 'User must be logged in');
    }

    const uid = context.auth.uid;
    const deviceInfo: DeviceInfo = data.deviceInfo;
 ...
 * - Jumpy coordinates
 */
export const detectGpsAnomaly = functions.https.onCall(async (data, context) => {
    if (!context.auth) {
        throw new functions.https.HttpsError('unauthenticated', 'User must be logged in');
    }
...
async function flagUser(uid: string, reason: string) {
    await db.collection('users').doc(uid).update({
